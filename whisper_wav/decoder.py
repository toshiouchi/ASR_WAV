...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_outs, tgt, memory_padding_mask, tgt_padding_mask):
        ...
        self_mask = self_mask.expand( (-1, self.dec_num_heads, -1, tgt.size(1)))
        #print( "size of self_mask:", self_mask.size() )
        #print( "memory_padding_mask size:", memory_padding_mask.size() )
        cross_mask = memory_padding_mask[:,None,None,:]
        cross_mask = cross_mask.expand((-1,self.dec_num_heads, tgt.size(1), -1))
        #print( "size of cross_mask:", cross_mask.size() )

        for i, block in enumerate( self.blocks ):
            x, attn1, attn2 = block(tgt, encoder_outs, self_mask, cross_mask)
