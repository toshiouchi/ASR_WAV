...
if __name__ == "__main__":
    ...
    # 学習データの特徴量(wav2.scp)が存在するディレクトリ
    feat_dir_train = '../01compute_features/fbank/train_large'
    wav_dir_train = '../data/label/train_large'
    # 開発データの特徴量(wav2.scp)が存在するディレクトリ
    feat_dir_dev = '../01compute_features/fbank/dev'
    wav_dir_dev = '../data/label/dev'
    ...
    wav_scp_train = os.path.join(wav_dir_train, 'wav2.scp')
    feat_scp_dev = os.path.join(feat_dir_dev, 'feats.scp')
    wav_scp_dev = os.path.join(wav_dir_dev, 'wav2.scp')
    ...
    token_list_path = os.path.join(exp_dir, 'data', unit,
                                   'token_list')
    # 学習結果を出力するディレクトリ
    output_dir = os.path.join(exp_dir, unit+'_model_trf_nar')
    ...
    #fe_conv_channel = [512,512,512,512,512]
    #fe_conv_channel = [512,512,512,512,512,512]
    fe_conv_kernel = [10,3,3,3,3,2,2]
    #fe_conv_kernel = [10,3,3,3,3]
    #fe_conv_kernel = [10,3,3,3,3,2]
    fe_conv_stride = [5,2,2,2,2,2,2]
    #fe_conv_stride = [5,2,2,2,2]
    #fe_conv_stride = [5,2,2,2,2,2]
    fe_conv_dropout = 0.1
    fe_out_dim = 512

    # Encoder の Attention に入力するための conv 層の設定
    conv_layers = 3
    conv_in_channels = [512,512,512]
    ...
    train_dataset = SequenceDataset(wav_scp_train,
                                    label_train,
                                    pad_id,
                                    enc_pad_id,
                                    )
    #train_dataset = SequenceDataset(wav_scp_dev,
    #                                label_dev,
    #                                )
                                    #feat_mean,
                                    #feat_std)
    ...
    # 開発データのデータセットを作成する
    dev_dataset = SequenceDataset(wav_scp_dev,
                                  label_dev,
                                  pad_id,
                                  enc_pad_id
                                  )
    ...
    for epoch in range(max_num_epoch):
        ...
        for phase in ['train', 'validation']:
            ...
            for (features, labels, feat_lens,
                ...
                #print( "torch.argmax( logits[0], dim = 1 ):",torch.argmax(logits[0], dim = 1 ) )
                #print( "labels[0]:", labels[0])
                loss = criterion(F.log_softmax( logits, dim=2 ).transpose(0,1), labels, logits_lengths, target_lengths  )
                ...
                if evaluate_error[phase]:
                    ...
                    for n in range(logits.size(0)):
                        # 各ステップのデコーダ出力を得る
                        #_, hyp_per_step = torch.max(logits[n], 1)
                        # numpy.array型に変換
                        #hyp_per_step = hyp_per_step.cpu().numpy()
                        preds = torch.argmax( logits, dim = 2 )
                        hyp_per_step = preds[n]
                        hyp_per_step = hyp_per_step.detach().cpu().numpy()
                        #hyp_per_step = hyp_per_step.astype( np.int32 )
                        # 認識結果の文字列を取得
                        #hypothesis = []
                        #label_len = label_lens[n].long()
                        #print( "label_lens[n]:{}".format( label_lens[n] ))
                        #print( "hyp_per_step[:label_lens[n]]:{}".format( hyp_per_step[:label_lens[n]] ))
                        #for m in hyp_per_step[:label_lens[n]]:
                        ##for m in hyp_per_step[:label_len]:
        ...
        for (features2, labels2, feat_lens2,
            ...
            # モデルの出力を計算(フォワード処理)
            logits2 = model(features2.to(device), feat_lens2.to(device) )


            # バッチ内の1発話ごとに以下の処理を行う
            n_batch = 0
            for n in range(logits2.size(0)):
                if n_batch >= 4:
                    break
                n_batch += 1
                ...
                preds2 = torch.argmax( logits2, dim = 2 )
                hyp_per_step2 = preds2[n]
                hyp_per_step2 = hyp_per_step2.detach().numpy()
                hypothesis2 = ctc_simple_decode( hyp_per_step2, token_list )
                    
                ## 認識結果の文字列を取得
                #hypothesis3 = []
                #for m in hyp_per_step3[:dec_target_maxlen]:
                #    #print( "m:{}".format( m ) )
                #    if m == eos_id:
                #        break
                #    hypothesis3.append(token_list[m])
                                
                print( "train    hypothesis:  ", *hypothesis2, sep="" )
                #print( "train ar hypothesis:  ", *hypothesis3, sep="" )
        '''     
        n_dev = 0
        for (features2, labels2, feat_lens2,
                   label_lens2, utt_ids2) in dev_loader:
            #print( "labels2:", labels2 )
            #print( "utt_ids2:", utt_ids2 )
            #sys.exit()
            if n_dev >= 1:
                break
            n_dev += 1
            ...
            # モデルの出力を計算(フォワード処理)
            logits2 = model(features2.to(device), feat_lens2.to(device) )
            #outputs2, _ = model(features2.to(device), feat_lens2.to(device), labels2.to(device), label_lens2.to(device) )
            ...
            for n in range(logits2.size(0)):
                ...
                preds2 = torch.argmax( logits2, dim = 2 )
                hyp_per_step2 = preds2[n]
                hyp_per_step2 = hyp_per_step2.detach().cpu().numpy()
                hypothesis2 = ctc_simple_decode( hyp_per_step2, token_list )
                ...
                #print( "dev ar   hypothesis:  ", *hypothesis3, sep="" )
            #sys.exit()
