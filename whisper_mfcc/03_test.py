...
if __name__ == "__main__":
    ...
    feat_dir_test = '../01compute_features/mfcc/test'
    ...
    model1_dir = os.path.join(exp1_dir, unit+'_model_conv_nar')
    ...
    token_list_path = os.path.join(exp1_dir, 'data', unit,
                                   'token_list')
    # 学習結果を出力するディレクトリ
    output_dir = os.path.join(exp1_dir, unit+'_model_conv_nar')
    ...
    # CTC損失関数を呼び出す．
    # blankは0番目と定義する．
    criterion = nn.CTCLoss(blank=blank_id, reduction='mean')    
    ...
    with open(hypothesis_file, mode='w') as hyp_file, \
        ...
        for (features2, labels2, feat_lens2,
            ...
            label_lens2 += 2
            labels2 = labels2[:,:torch.max(label_lens2)]
            features2 = features2[:,:torch.max(feat_lens2)]

            # モデルの出力を計算(フォワード処理)
            with torch.no_grad():
                outputs3 = model(features2.to(device), feat_lens2.to(device) )
