...
class MyE2EModel(nn.Module):
    ...
    def forward(self,
        ...
        enc_out, memory_padding_mask = self.encoder(input_sequence,input_lengths, mask)
        ...
        tgt_padding_mask = F.interpolate( mask[:,None,:], size= dec_input.size(1) )[:,0,:]
        
        # デコーダに入力する
        #print( "dec_input.size:{}".format( dec_input.size() ))
        dec_out = self.decoder(enc_out, dec_input, memory_padding_mask[:,0,:,0], tgt_padding_mask )
        #print( "dec_out.size:{}".format( dec_out.size() ))
