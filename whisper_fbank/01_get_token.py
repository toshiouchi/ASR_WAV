...
def token_to_int(label_file_str,
    ...
    '''
    # 各ファイルを開く
    with open(label_file_str, mode='r', encoding="UTF-8") as label_in, \
             open(label_file_int, mode='w', encoding="UTF-8") as label_out, \
             open(unknown_list_file, mode='w', encoding="UTF-8") as unk_list:
        # ラベルファイルを一行ずつ読み込む
...
if __name__ == "__main__":
    ...
    for uid, unit in enumerate(unit_set):
        ...
        # 訓練データのラベルに存在するトークンを
        # token_listに登録する
        with open(label_train, encoding="UTF-8") as label_file:
            # 一行ずつ読み込む
        ...
        token_list.insert(0, blank_token)
        token_list.insert(0, pad_token)
